...
symunins()
{
    ...
    echo -ne '################################################ (100%)\r'
    echo ""
    echo -e "\rUninstalling Symantec ... \033[1;32mDONE \033[m"
}
...
if ! dpkg -s $pkgs >/dev/null 2>&1; then
	...
	echo -e "\rInstalling Mariadb ... \033[1;32mDONE \033[m"
else
	echo -e "\rMariadb Installed ... \033[1;32mDONE \033[m"
fi
}
...
if ! [ -x "$(command -v php5.6)"  ]; then
	echo -n "Php5.6 is installing ..."
	php5_6
	echo -e "\rPhp5.6 is installing ... \033[1;32mDONE \033[m"
	php_install
elif ! [ -x "$(command -v php7.0)"  ]; then
        echo -n "Php7.0 is installing ..."
	php7_0
	echo -e "\rPhp7.0 is installing ... \033[1;32mDONE \033[m"
	php_install
elif ! [ -x "$(command -v php7.1)"  ]; then
        echo -n "Php7.1 is installing ..."
	php7_1
	echo -e "\rPhp7.1 is installing ... \033[1;32mDONE \033[m"
	php_install
elif ! [ -x "$(command -v php7.2)"  ]; then
        echo -n "Php7.2 is installeing ..."
	php7_2
	echo -e "\rPhp7.2 is installing ... \033[1;32mDONE \033[m"
	php_install
...
if ! dpkg -s $pkgs >/dev/null 2>&1; then
	echo -n "Installing Nginx "
        apt-get install -y $pkgs >/dev/null
	echo -e "\rInstalling Nginx ... \033[1;32mDONE \033[m"
fi
}
...
pkgs='docker-compose'
if ! dpkg -s $pkgs >/dev/null 2>&1; then
        echo -n "Installing Missing Docker-compose ..."
	...
	chmod +x /usr/local/bin/docker-compose
       apt-get install -y $pkgs >/dev/null
        echo -e "\rInstalling Missing Docker-compose ...\033[1;32mDONE \033[m"
fi
}
...
if [ "$lan" = "y" ] || [ "$lan" = "Y" ] || [ "$lan" = "" ]; then
   ...
   stable" >/dev/null 2>&1
    ...
    echo -ne '################################################ (100%)\r'
    echo ""
	echo -e "\rLando Installing ...\033[1;32mDONE \033[m"
else 
...
docker(){
    ...
    echo -n "\033[1;32mFor Docker Installtion !!!\033[0m "
    echo ""
...
echo " "  
echo -n "\033[0;31mEnter Username :\033[0m "
read optr
#echo "$opt"
if [ -z "$optr" ]
then
	...
	echo  "Dir Created ... \033[1;32mDONE \033[m"
	cd projects/php/
	echo -n "Git Clone"
	git clone http://ashwin.m:aug%40123@rage2.git/core/docker-php.git ./ --quiet
	cd $user
	sleep 1;
	echo -e "\rGit Clone ... \033[1;32mDONE \033[m"
	sleep 1;
	echo -n "Permission Changing"
	chmod -R 777 projects/php/data/
	chmod -R 777 projects/php/docker/vhosts
	chown -R $per''$optr:$domain projects/
	echo -e "\rPermission changing ... \033[1;32mDONE \033[m"
	sleep 1;
	echo -n "Php is installing ..."
	php_install
	echo -e "\rPhp is installing ... \033[1;32mDONE \033[m"
	mariadb
	sleep 1;
	echo -n "Mariadb Stop and Disable \033[1;32mDONE \033[m"
	systemctl stop mysql --quiet
	systemctl disable mysql --quiet
	echo -e "\rMariadb Stop and Disable ... \033[1;32mDONE \033[m"
	sleep 1;
	nginx
	echo -n 'Nginx stop and disable '
	systemctl stop nginx --quiet
	systemctl disable nginx --quiet
	echo -e '\rNginx stop and disable ... \033[1;32mDONE \033[m'
	sleep 1;
	echo -n 'PHP stopping and disable '
	...
	echo -e '\rPHP stopping and disable ... \033[1;32mDONE \033[m'
	sleep 1;
	dockercompose
	sleep 1;
